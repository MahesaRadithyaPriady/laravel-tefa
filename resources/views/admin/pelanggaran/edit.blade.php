...
<head>
  ...
  <title>Edit Pelanggaran - SMKN 4 TASIKMALAYA</title>
    ...
    <div class="max-w-4xl mx-auto py-8 px-4 sm:px-6 lg:px-8">
      <div class="bg-white shadow-md rounded-lg overflow-hidden">
        <div class="px-6 py-4 bg-indigo-600">
          <h2 class="text-xl font-bold text-white">Edit Pelanggaran</h2>
        </div>
        ...
        <div class="p-6">
          ...
          <!-- Form -->
          <form action="{{ route('pelanggaran.update', $pelanggaran->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="bg-indigo-50 p-4 rounded-lg mb-6">
              ...
              <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div class="form-group md:col-span-2">
                  ...
                  <textarea id="jenis" name="jenis" rows="4" class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-indigo-500 focus:border-indigo-500">{{ $pelanggaran->jenis }}</textarea>
                </div>

                <div class="form-group md:col-span-2">
                  <label for="konsekuensi" class="form-label">Konsekuensi <span class="text-red-500">*</span></label>
                  <textarea id="konsekuensi" name="konsekuensi" rows="4" class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-indigo-500 focus:border-indigo-500">{{ $pelanggaran->jenis }}</textarea>
                </div>

                <div class="form-group">
                  <label for="poin" class="form-label">Poin <span class="text-red-500">*</span></label>
                  <input type="text" id="poin" name="poin" value="{{ $pelanggaran->poin }}" class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-indigo-500 focus:border-indigo-500" required>
                </div>
              </div>
            </div>
