...
<head>
  ...
  <title>Edit Akun - SMKN 4 TASIKMALAYA</title>
    ...
    <div class="max-w-4xl mx-auto py-8 px-4 sm:px-6 lg:px-8">
      <div class="bg-white shadow-md rounded-lg overflow-hidden">
        ...
        <div class="p-6">
          <!-- Back button -->
          <div class="mb-6">
            <a href="{{ route('akun.index') }}" class="inline-flex items-center text-indigo-600 hover:text-indigo-800">
          ...
          <div class="mb-6 bg-green-50 border border-green-400 text-green-700 px-4 py-3 rounded relative" role="alert">
          ...
          <div class="bg-indigo-50 p-4 rounded-lg mb-6">
            <h3 class="text-lg font-medium text-indigo-800 mb-4 pb-2 border-b border-indigo-200">Data Akun</h3>
            
            <form action="{{ route('akun.update', $akun->id) }}" method="POST" enctype="multipart/form-data" class="space-y-4">
              @csrf
              @method('PUT')
              ...
              <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div class="form-group">
                  ...
                  <input type="text" id="name" name="name" value="{{ $akun->name }}" class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-indigo-500 focus:border-indigo-500" required>
                </div>

                <div class="form-group">
                  <label for="usertype" class="form-label">Hak Akses <span class="text-red-500">*</span></label>
                  <select id="usertype" name="usertype" class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-indigo-500 focus:border-indigo-500" required>
          ...
          <div class="bg-indigo-50 p-4 rounded-lg mb-6">
            <h3 class="text-lg font-medium text-indigo-800 mb-4 pb-2 border-b border-indigo-200">Update Email</h3>
            
            <form action="{{ route('updateEmail', $akun->id) }}" method="POST" enctype="multipart/form-data" class="space-y-4">
              @csrf
              @method('PUT')
              
              <div class="form-group">
                <label for="email" class="form-label">Email Address <span class="text-red-500">*</span></label>
                <input type="email" id="email" name="email" value="{{ $akun->email }}" class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-indigo-500 focus:border-indigo-500" required>
              </div>
          ...
          <div class="bg-indigo-50 p-4 rounded-lg mb-6">
            <h3 class="text-lg font-medium text-indigo-800 mb-4 pb-2 border-b border-indigo-200">Update Password</h3>
            
            <form action="{{ route('updatePassword', $akun->id) }}" method="POST" enctype="multipart/form-data" class="space-y-4">
              @csrf
              @method('PUT')
